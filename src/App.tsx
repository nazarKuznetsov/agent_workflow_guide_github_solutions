...
const promptTemplates: PromptBlock[] = [
  {
...
const stagedPrompts = (brainstormCopy: string, plannerPrompt: PromptBlock): PromptBlock[] => [
  {
    ...promptTemplates[0],
    copy: brainstormCopy,
  },
  {
    ...
    copy: `Planner setup prompt for this instruction type: ${plannerPrompt.copy}`,
  },
  {
    ...promptTemplates[2],
    title: "Orchestrator prompt",
    copy: "Orchestrator queue prompt for applying setup, project state, readiness, worker packets, and Human action required fallback.",
  },
  {
    ...promptTemplates[3],
    title: "Worker prompt",
    copy: "Worker delivery prompt. Reference only: this is generated by Orchestrator after the issue passes the readiness gate.",
  },
  {
    ...promptTemplates[4],
    title: "QA verification prompt",
    copy: "Reference only: Worker fills evidence, and Orchestrator verifies the QA block before Review or Done.",
  },
];
...
const instructionTypes: InstructionType[] = [
  {
    id: "setup",
    eyebrow: "Self-contained setup guide",
    title: "New project setup / настройка нового проекта.",
    copy: "Используй этот блок как пошаговую инструкцию: сначала включи GitHub-функции, затем скопируй шаблоны, создай Project schema и только после этого запускай агентную очередь.",
    steps: newProjectSteps,
    promptHeading: "New project prompts",
    promptCopy: "Промпты для полного запуска нового репозитория: idea summary, Planner setup, Orchestrator automation, Worker delivery и QA verification.",
    prompts: stagedPrompts("Первый чат для нового проекта. Он формирует idea summary для Planner; Canonical MD остается отдельным блоком.", promptTemplates[1]),
  },
  {
    eyebrow: "Migration path",
    title: "Existing project integration / внедрение в готовый проект.",
    copy: "В существующем проекте задача не в том, чтобы переписать backlog. Сначала вводится единый ready gate, затем пилотируется один полный цикл issue to PR to Done.",
    steps: existingProjectSteps,
    promptHeading: "Existing project prompts",
    promptCopy: "Промпты для аккуратной миграции без поломки локальных правил, CI, PR template, release constraints и текущего backlog.",
    prompts: stagedPrompts("Первый чат фиксирует текущий процесс, ограничения и candidate pilot issues для Planner.", promptTemplates[5]),
  },
  {
    eyebrow: "Linear adoption path",
    title: "Existing project with Linear.",
    copy: "Если продуктовая очередь уже в Linear, не удаляй ее сразу. Сначала зафиксируй Linear workflow audit, сделай Linear-to-GitHub mapping и запускай Workers только из mirrored GitHub Issues.",
    steps: existingLinearProjectSteps,
    promptHeading: "Linear project prompts",
    promptCopy: "Промпты для переходного режима: Linear остается product/backlog source, а GitHub Issue и Project становятся execution source для agent delivery.",
    prompts: stagedPrompts("Первый чат описывает текущий Linear workflow, pilot candidates и границу между Linear backlog и GitHub execution.", promptTemplates[6]),
  },
  {
    eyebrow: "Bootstrap path",
    title: "Existing project without agent workflow.",
    copy: "Если нет Linear, GitHub Project queue, labels или шаблонов, начинай с No-tooling bootstrap: Planner пишет GitHub Setup Packet, Orchestrator applies setup, и только потом появляется первый pilot Worker PR.",
    steps: noToolingProjectSteps,
    promptHeading: "No-tooling bootstrap prompts",
    promptCopy: "Промпты для проекта без агентного процесса: сначала operating system, затем setup-sized issues и только потом product delivery.",
    prompts: stagedPrompts("Первый чат описывает текущий проект, bootstrap outcome, access blockers и safe pilot issue для Planner.", promptTemplates[7]),
  },
];

const promptMapItems: Card[] = [
  {
    title: "Planner setup prompt",
    copy: "Теперь это Planner prompt внутри нужного типа инструкции.",
  },
  {
    title: "Orchestrator queue prompt",
    copy: "Теперь это Orchestrator prompt внутри нужного типа инструкции.",
  },
  {
    title: "Worker delivery prompt",
    copy: "Теперь это Worker prompt внутри нужного типа инструкции; человек обычно не копирует его вручную.",
  },
];
...
no open blockers`}</pre>
      ...
      {instructionTypes.map((instruction, index) => (
        <section className="setup-playbook section-band" id={instruction.id} key={instruction.title}>
          <div className="section-heading">
            <p className="eyebrow">{instruction.eyebrow}</p>
            <h2>{instruction.title}</h2>
            <p>{instruction.copy}</p>
          </div>
          <div className="setup-grid">
          ...
          <div className="section-heading prompt-heading">
            ...
            <h2>{instruction.promptHeading}</h2>
            <p>{instruction.promptCopy}</p>
          </div>
          <div className="prompt-grid" id={index === 0 ? "prompts" : undefined}>
            {instruction.prompts.map((prompt) => (
              <article className="prompt-card" key={`${instruction.title}-${prompt.title}`}>
                <div>
                  <h3>{prompt.title}</h3>
                  <p>{prompt.copy}</p>
                </div>
      ...
      <section className="prompts section-band">
        <div className="section-heading">
          <p className="eyebrow">Prompt map</p>
          <h2>Полные промпты теперь находятся под типами инструкции.</h2>
          <p>
            Canonical MD остается отдельной секцией. Этот блок сохраняет старые названия как карту к новым staged
            prompt cards.
          </p>
        </div>
        <div className="setup-grid">
          {promptMapItems.map((item) => (
